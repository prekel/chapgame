...
\begin{centering}
    \begin{longtable}{|l|l|l|}
        ...
        \multirow{2}{*}{Bisection}                            & \href{https://github.com/prekel/chapgame/blob/master/lib/solver/bisection.ml}{lib/solver/bisection.ml}                                   & \multirow{2}{*}{\hyperref[bisectiondescr]{Бисекция}}                    \\*
...
разбиения списка значений на промежутки, вычисления разности между правым и левым значением.\label{intervaldescr}
...
а на предыдущем шаге мы исключили одночлен с нулевой степенью)>>.\label{polynomialdescr}
...
позволяющий находить значения на бесконечном и полубесконечном промежутке, который описан в пункте~\ref{bisection}.\label{bisectiondescr}
...
Иначе, вычислить производную многочлена и найти её корни; разбить список корней на промежутки и методом бисекции найти корни на промежутках>>.\label{equationdescr}
...
его значение при \(x = 24\) и \(y = 4\).\label{exprdescr}
...
многочлен~(\ref{samplepolyexpr}) и подставляются \(x = 24\) и \(y = 4\).\label{exprpolydescr}
...
\textbf{Точка} и \textbf{Линия}. Точка определена как через её координаты по осям \(X\) и \(Y\).
...
\end{Underequation}\label{pointlinedescr}

\textbf{Значения}. Служит для хранения значений величин. Определены через иммутабельный словарь, ключами которого являются
идентификаторы величин, а значениями являются значения величин. \TODO\label{valuesdescr}

\textbf{Правило}. Определяет выражения-многочлены для положения и скорости тела.
...
Остальные правила указывают на тот же набор правил, в котором определены.\label{rulesdescr}
...
и возвращающую новый набор тел.\label{bodydescr}

\textbf{Обнаружение столкновений}. Задача функций в этом модуле вычислять время столкновения.
...
(поменяв местами точки \(A\) и \(B\) в условии) и подобным образом проверяется столкновение с лучом.\label{collisiondetectiondescr}
...
\textbf{Обработка ударов}. Этот модуль реализует формулы~(\ref{collisionhandle}) и~(\ref{collisionhandleafterlim}) и
предоставляет 3 функции:\label{collisionhandledescr}
...
\textbf{Действие}. Этот модуль описывает тип действия, которое можно применить к модели.
...
так что подбор оптимальных параметров расчёта ложится на пользователя. Однако, если тела могут продолжить двигаться и между ними не будет проииходить столкновений,
...
\textbf{Модель}. \TODO\label{modeldescr}

\textbf{Движок}. \TODO\label{enginedescr}
