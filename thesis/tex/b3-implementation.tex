...
Это позволило одновременно определить кроме прямой, ещё и отрезок и луч. При этом определены функции для получения коэффициентов общего уравнения прямой по формуле~(\ref{p1p2toabc}).
...
\TODO
В случае обнаружения столкновения с лучом или отрезком, кроме вычисления времени столкновения через уравнение,
требуется дополнительно проверить, что точна столкновения находится на этом луче или отрезка, потому что она может находиться \TODO
